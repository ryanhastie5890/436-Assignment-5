...
class GameManager
{
    ...
    public function buildStatusList()
    {
        ...
        foreach ($players as $row) {
            ...
            if ($x && $o) {
                if (isset($statusMap[$x]))
                    $statusMap[$x] = ['state' => 'playing-X', 'opponent' => $o];
                if (isset($statusMap[$o]))
                    $statusMap[$o] = ['state' => 'playing-O', 'opponent' => $x];
            } else if ($x && !$o) {
                if (isset($statusMap[$x]))
                    $statusMap[$x] = ['state' => 'waiting-X', 'opponent' => null];
            } else if ($o && !$x) {
                if (isset($statusMap[$o]))
                    $statusMap[$o] = ['state' => 'waiting-O', 'opponent' => null];
            }
        }
    ...
    public function handleLogin($screenname)
    {
        $logged = $this->getLoggedIn();
        if ($logged === false)
            return ['action' => 'ERROR', 'message' => 'DB error'];
        foreach ($logged as $r) {
            if ($r['screenname'] === $screenname)
                return ['action' => 'screenname-unavailable'];
        }

        if (!$this->insertLoggedIn($screenname))
            return ['action' => 'ERROR', 'message' => 'DB insert failed'];
    ...
    // LOGOUT
    public function handleLogout($screenname)
    {
        if (!$screenname)
            return false;
        $res = $this->removeLoggedIn($screenname);
        return $res;
    }

    public function removePlayerRowsByScreenname($screenname)
    {
        $rows = $this->findPlayerRowByName($screenname);
        if ($rows === false)
            return false;
        foreach ($rows as $r) {
            $this->deletePlayerById($r['id']);
        }
        return true;
    }
    ...
    public function handleNewGame($screenname, $choice)
    {
        $logged = $this->getLoggedIn();
        if ($logged === false)
            return ['action' => 'ERROR', 'message' => 'DB error 1'];
        $found = false;
        foreach ($logged as $r)
            if ($r['screenname'] === $screenname)
                $found = true;
        if (!$found)
            return ['action' => 'ERROR', 'message' => 'Not logged in'];

        $prow = $this->findPlayerRowByName($screenname);
        if ($prow === false)
            return ['action' => 'ERROR', 'message' => 'DB error 2'];
        if (count($prow) > 0)
            return ['action' => 'ERROR', 'message' => 'Already in players table'];

        if ($choice === 'X')
            $ok = $this->insertPlayer($screenname, null);
        else
            $ok = $this->insertPlayer(null, $screenname);

        if (!$ok)
            return ['action' => 'ERROR', 'message' => 'DB insert failed'];
        $list = $this->buildStatusList();
        if ($list === false)
            return ['action' => 'ERROR', 'message' => 'DB error 3'];
    ...
    public function handleJoin($from, $to)
    {
        ...
        if (!$xName && $oName)
            ...
            return ['action' => 'ERROR', 'message' => 'Invalid waiting row'];

        $updOk = $this->updatePlayerById($waitingRow['id'], $xName, $oName);
        if (!$updOk)
            return ['action' => 'ERROR', 'message' => 'DB update failed'];
    ...
    {
        if (isset($this->games[$rowId]))
            return $this->games[$rowId];
        return null;
    }
